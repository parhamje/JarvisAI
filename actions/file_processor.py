...
class _GeminiCompat:
    """Thin wrapper so all existing model.generate_content() calls keep working."""
    def __init__(self, api_key: str):
        self._client = google_genai.Client(api_key=api_key)

    def generate_content(self, content):
        # content can be a string, a list of strings/PIL images, etc.
        if isinstance(content, str):
            ...
            for item in content:
                if isinstance(item, str):
                    ...
                    # PIL Image — convert to bytes and wrap
                    import io
                    buf = io.BytesIO()
...
def _gemini_client() -> _GeminiCompat:
    return _GeminiCompat(api_key=_get_api_key())
