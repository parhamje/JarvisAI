...
def _screen_find(description: str) -> tuple[int, int] | None:
    ...
    try:
        ...
        prompt = (
            f"This is a screenshot of a computer screen. "
            f"The screen resolution is exactly {w} x {h} pixels. "
            f"Find the UI element: '{description}'. "
            f"Reply with ONLY the center coordinates in actual screen pixels as: x,y "
            f"where x is between 0 and {w}, and y is between 0 and {h}. "
            f"Example reply: 954,540 "
        ...
        print(f"[ComputerControl] screen_find raw response: {text!r}")
        ...
        if match:
            ...
            # If Gemini returned normalized 0-1 values instead of pixels, rescale
            if x <= 1 and y <= 1:
                x = int(x * w)
                y = int(y * h)
            ...
            x = max(0, min(x, w - 1))
            y = max(0, min(y, h - 1))
            print(f"[ComputerControl] screen_find '{description}' -> ({x}, {y})")
            return x, y
